...
## Freeze 2 — Paper 1, after freeze lift 1

**Freeze-1 was `ce427a8`** and its block above stands as the record of what was
frozen then. The freeze was lifted **once**, on instruction, for four experiments
answering four reviewer objections, and is now closed again.

**§1–§25 above are Paper 1's evidence base**, together with
`docs/literature-corrections.md` #1–#17, `docs/validation.md`, and the four
pre-registrations in `docs/preregistration/`. Every number the paper quotes must
be traceable to one of them, and every number in §13 that a script can recompute
is recomputed by `scripts/verify_numbers.py` with `scripts/check_section13.py`
joining the two — **114 MATCH, 0 MISMATCH** at this freeze.

**What the lift added.** Four experiments (§§22–25), each pre-registered before
its sweep ran: a *searched* S = 3 pursuer row, capability flatness at n ∈ {10, 50},
the tuning control at a second and third correlation length, and a pseudo-reality
robustness check. Twenty-seven new §13 rows, 163–189. Two claims upgraded, one
split by swarm size, one reworded, three new ids, six new limitations.

**What the lift corrected.** Two §13 row pairs that did not reproduce from the
configs as committed (F1, F2 — neither changes a claim), one group disclosed as
non-regenerable (F3), and **two simulator defects of the same class**, each fixed
so that every published run is bit-identical: `p_lock` was a per-step Bernoulli
standing in for a per-second rate, and `wheel_noise` was a fixed-σ velocity noise
whose accumulated diffusion scaled with `dt`. Twelve and eleven results files
respectively regenerated byte-for-byte identical.

**Freeze-2 commit: `09d0d693ca54d8f96a6eb4cc5ca799f4eae26744`** ("Freeze lift 1: searched S3 pursuer row, n-scaling,
second lambda, pseudo-reality check; re-frozen"). The annotated tag `freeze-2`
points at it. *The tag exists in this clone but could not be pushed: the session's
GitHub credential is scoped to branch refs and refuses `refs/tags/*` with HTTP 403.
The commit is pushed; the hash above is what identifies the freeze, and anyone with
push rights can recreate the tag from it.*

**No further simulation runs are permitted in this branch for Paper 1.** The
experimental record is closed at the commit above. Consolidation and writing
may read the record; they may not add to it, re-run any part of it, or recompute a
quantity that is not already in it. Anything a later phase needs and cannot find
is written down as a source gap and left as one.

**The follow-up list above stands**, with these added by the lift:

* **A λ-matched decomposition.** Experiment 3 evaluated λ = 0.10-trained rows at
  0.05 and 0.20 m, so it measured transfer. Searching at each λ is a different and
  much more expensive experiment and is the one that would separate "the terrain
  term is real everywhere" from "part of it is a mismatch penalty" (L34).
* **A second, structurally different optimiser.** Carried forward from §21 and
  untouched: three seeds of sep-CMA-ES share a diagonal covariance.
* **Searching the S = 4 rows at n ≠ 20.** A1's grade at n ∈ {10, 50} is limited by
  transferred candidates, not by the data (L33, §23).
* **Why the aggregation dynamics are timestep-sensitive in the enumerated row and
  not in the searched one** (L31). Every published number is at `dt = 0.1`; what
  is limited is the claim of timestep-independence.
* **A real reality-gap study.** The pseudo-reality family perturbs implementation
  choices, not the physics being modelled. §12.3 still records that no hardware,
  no ARGoS and no second simulator exist.
